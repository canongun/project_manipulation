...
class GroundTruthListenerBaseLinkNode:
    """
    The Multirobot Topic Listener
    """

    def __init__(self):
        rospy.init_node('ground_truth_listener_base_link_server')

        self.pose = Odometry()

        self.message_received = False  # Flag to indicate whether a message has been received

        rospy.Service('ground_truth_listener_base_link',    # service name
    ...
    def ground_truth_listener(self, req):
        ...
        if req.start:
            ...
            rospy.Subscriber('/ground_truth/base_link_state', Odometry, self.callback)
...
def main():
    print("Listening to 'ground_truth/base_link_state' topic")
    node = GroundTruthListenerBaseLinkNode()
    node.run()
